...
def detect_reference_objects(image_cv2, max_dim=1280, min_area_frac=REFERENCE_MIN_AREA):
    ...
    load_models_if_needed()   # SAM is not needed for label-only detection
...
def process_scene_pipeline(image: Image.Image, room_id: str, filename: str, masks_folder: str, generated_folder: str, server_base_url: str):
    ...
    inputs = processor(images=process_image, return_tensors="pt").to(device)
    with torch.no_grad():
        outputs = segmenter(**inputs)

    # --- Clear Cache after heavy forward pass ---
    torch.cuda.empty_cache()
    print("➡ [INFO] EoMT segmentation completed; GPU cache cleared.")

    panoptic_result = processor.post_process_panoptic_segmentation(
        outputs, target_sizes=[process_image.size[::-1]]
    )[0]
    ...
    # SAM SETUP
    image_cv = cv2.cvtColor(np.array(process_image), cv2.COLOR_RGB2BGR)
    ...
    for hotspot in hotspots:
        ...
        try:
            ...
            if _birefnet_occluder_mask is not None:
                ...
                mask_uint8 = cv2.bitwise_or(mask_uint8, reveal_occluder_holes(_birefnet_occluder_mask, pre_occluder_mask))
            ...
            if seg_class == "curtain" and fg_full is not None:
                ...
                mask_uint8 = cv2.bitwise_or(mask_uint8, reveal_occluder_holes(fg_full, pre_occluder_mask))
        except Exception as e:
